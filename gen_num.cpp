...
int max_w, max_h, tot_size, tot_num, obj;
...
int main(int argc, char *argv[])
{
    if (argc == 5)
    {
		...
		if (max_w < 5 || max_h < 5 || obj >= tot_size || obj < 1 ||  tot_num < 1) {
			printf("Usage: ./gen <width> <height> <number> <object> \
			(w & h must >= 5, 0 < obj number < total size, map number >= 1.)\n");
			return 0;
		}
